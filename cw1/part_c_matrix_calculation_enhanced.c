...
const char* SIZE_LABELS[MAX_MATRIX_SIZES] = {"50x50", "100x100", "500x500", "1000x1000"};
...
double** allocate_matrix_aligned(int rows, int cols) {
    ...
    for (int i = 0; i < rows; i++) {
        matrix[i] = (double*)aligned_alloc(64, cols * sizeof(double));  // 64-byte alignment
        if (!matrix[i]) {
            // Cleanup on failure
            for (int j = 0; j < i; j++) {
                free(matrix[j]);
...
double matrix_multiply_cache_optimized(double **A, double **B, double **C, int size, int num_threads) {
    omp_set_num_threads(num_threads);
    double start_time = omp_get_wtime();
...
void calculate_statistics(PerformanceData *data) {
    ...
    for (int i = 0; i < NUM_RUNS; i++) {
        ...
        if (data->times[i] < data->min_time) data->min_time = data->times[i];
        if (data->times[i] > data->max_time) data->max_time = data->times[i];
    }
    ...
    long long operations = 2LL * data->matrix_size * data->matrix_size * data->matrix_size;
    data->gflops = operations / (data->avg_time * 1e9);
}

/* Print detailed timing table */
void print_timing_table(PerformanceData *outer_data, PerformanceData *inner_data, 
                       int matrix_size, const char* size_label) {
    printf("\n================================================================\n");
    ...
    for (int t = 0; t < MAX_THREAD_CONFIGS; t++) {
        ...
        printf("%8.6f | %8.6f | %8.6f | %8.6f | %6.2f\n",
               outer_data[t].avg_time, outer_data[t].min_time, outer_data[t].max_time,
    ...
    for (int t = 0; t < MAX_THREAD_CONFIGS; t++) {
        ...
        printf("%8.6f | %8.6f | %8.6f | %8.6f | %6.2f\n",
               inner_data[t].avg_time, inner_data[t].min_time, inner_data[t].max_time,
               inner_data[t].std_deviation, inner_data[t].gflops);
    }
}

/* Print ASCII performance graph */
void print_performance_graph(PerformanceData *outer_data, PerformanceData *inner_data, 
    ...
    double max_time = 0.0;
    for (int t = 0; t < MAX_THREAD_CONFIGS; t++) {
        if (outer_data[t].avg_time > max_time) max_time = outer_data[t].avg_time;
        if (inner_data[t].avg_time > max_time) max_time = inner_data[t].avg_time;
    }
    ...
    for (int t = 0; t < MAX_THREAD_CONFIGS; t++) {
        ...
        for (int i = 0; i < outer_bar_length; i++) printf("█");
        for (int i = outer_bar_length; i < 25; i++) printf(" ");
        printf(" %.6f | ", outer_data[t].avg_time);
        
        // Inner loop bar
        int inner_bar_length = (int)((inner_data[t].avg_time / max_time) * 25);
        for (int i = 0; i < inner_bar_length; i++) printf("█");
        for (int i = inner_bar_length; i < 25; i++) printf(" ");
...
void run_comprehensive_analysis(int matrix_size, const char* size_label) {
    ...
    for (int t = 0; t < MAX_THREAD_CONFIGS; t++) {
        ...
        for (int run = 0; run < NUM_RUNS; run++) {
            outer_data[t].times[run] = matrix_multiply_outer_parallel(A, B, C, matrix_size, threads);
        }
        calculate_statistics(&outer_data[t]);
        
        // Test inner loop parallelization
        for (int run = 0; run < NUM_RUNS; run++) {
            inner_data[t].times[run] = matrix_multiply_inner_parallel(A, B, C, matrix_size, threads);
        }
    ...
    for (int t = 0; t < MAX_THREAD_CONFIGS; t++) {
        if (outer_data[t].avg_time < inner_data[t].avg_time) {
            double improvement = (inner_data[t].avg_time - outer_data[t].avg_time) / inner_data[t].avg_time * 100;
            printf("%d threads: Outer Loop wins by %.1f%% (%.6fs vs %.6fs)\n",
                   THREAD_COUNTS[t], improvement, outer_data[t].avg_time, inner_data[t].avg_time);
        } else {
            double improvement = (outer_data[t].avg_time - inner_data[t].avg_time) / outer_data[t].avg_time * 100;
            printf("%d threads: Inner Loop wins by %.1f%% (%.6fs vs %.6fs)\n",
                   THREAD_COUNTS[t], improvement, inner_data[t].avg_time, outer_data[t].avg_time);
        }
    }
...
void generate_comprehensive_report() {
    ...
    printf("\nOUTER LOOP PARALLELIZATION:\n");
    printf("IMPLEMENTATION: #pragma omp parallel for applied to the outermost loop (i-loop)\n");
    ...
    printf("\nINNER LOOP PARALLELIZATION:\n");
    printf("IMPLEMENTATION: #pragma omp parallel for applied to the middle loop (j-loop)\n");
    printf("CHARACTERISTICS:\n");
    printf("✓ Advantages:\n");
    printf("  • Fine-grained parallelism\n");
    printf("  • Good for smaller matrices where outer loop has few iterations\n");
    printf("✗ Disadvantages:\n");
    printf("  • Higher synchronization overhead (parallel region created n times)\n");
    ...
    printf("• Memory access patterns affect performance more than parallelization overhead\n");
    ...
    printf("• Beyond core count: diminishing returns due to resource contention\n");
...
int main() {
    ...
    for (int i = 0; i < MAX_MATRIX_SIZES; i++) {
        ...
        if (i < MAX_MATRIX_SIZES - 1) printf(", ");
    }
    printf("\n• Thread counts: ");
    for (int i = 0; i < MAX_THREAD_CONFIGS; i++) {
        printf("%d", THREAD_COUNTS[i]);
        if (i < MAX_THREAD_CONFIGS - 1) printf(", ");
    }
    printf("\n• Runs per configuration: %d\n", NUM_RUNS);
    printf("• Total tests: %d matrix sizes × %d thread counts × 2 methods × %d runs = %d tests\n",
