...
#pragma omp for nowait
    ...
    // Without 'nowait', threads would wait here until ALL finished the first
    // loop. With 'nowait', threads finishing the first loop can immediately
    // start the second.
...
#pragma omp for nowait // Implicit barrier removed here too
    for (i = 0; i < m; i++) {
      ...
      if (i < n) {          // Check bounds for a[i]
        ...
        printf(
            "[NOWAIT] Thread %d skipping y[%d] due to boundary check (m > n)\n",
            tid, i);
      }
    }
    // Implicit barrier at the end of the parallel region ensures all threads
    // finish both loops (or parts they were assigned) before the function
    // returns.
...
#pragma omp for // Implicit barrier AT THE END of this loop
    for (i = 0; i < m; i++) {
      ...
      if (i < n) {          // Check bounds for a[i]
        ...
        printf(
            "[WAIT] Thread %d skipping y[%d] due to boundary check (m > n)\n",
            tid, i);
      }
    }
    // <<< Implicit barrier here.
...
int main(void) {
  ...
  int m = 12; // Size for array 'y' (loop 2 goes up to m-1). Set m > n to test
              // boundary.
  ...
  // Let's make 'a' large enough for both access patterns. Max index is max(n-1,
  // m-1). Size = max(n, m)
  int a_size = (n > m) ? n : m;
  ...
  // Set thread count, e.g., 4. You can change this or use environment variable
  // OMP_NUM_THREADS
